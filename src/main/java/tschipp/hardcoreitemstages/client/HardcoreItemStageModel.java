...
import tschipp.hardcoreitemstages.Config;
import tschipp.hardcoreitemstages.Tags;
...
public class HardcoreItemStageModel implements IBakedModel
{
	...
	public static class OverrideList extends ItemOverrideList
	{
		...
		{
			if (newmodel == null)
				newmodel = Minecraft.getMinecraft().getRenderItem().getItemModelMesher().getModelManager().getModel(new ModelResourceLocation(Tags.MOD_ID + ":unknown", "inventory"));

			if (!stack.isEmpty())
			{

                String itemId = stack.getItem().getRegistryName().toString();
                for (String excluded : Config.excludedItemsModels) {
                    if (excluded.equals(itemId)) {
