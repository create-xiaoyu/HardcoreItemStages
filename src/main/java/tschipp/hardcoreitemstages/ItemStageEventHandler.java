	...
	public static void onModelBakeEvent(ModelBakeEvent event)
	{
		if (Config.hideUnknownItems)
		{
			...
			for (ModelResourceLocation key : registry.getKeys())
			{
                ...
                for (String excludedItem : Config.excludedItemsModels) {
                    ResourceLocation itemRL = new ResourceLocation(excludedItem);
                    // 比较namespace和path，注意：ModelResourceLocation的变体部分我们不考虑
                    if (itemRL.getNamespace().equals(key.getNamespace()) && itemRL.getPath().equals(key.getPath())) {
                        excluded = true;
                        break;
                    }
                }
